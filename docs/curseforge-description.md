...
- **Undo and redo that survive restarts.** Every import is recorded in the world; `/meshtiles list`, `/meshtiles undo <id>`,
  `/meshtiles redo <id>`.
- **Blender add-on.** Pick the Minecraft block, detail and placement order for every material right in Blender, with
  block colours in the viewport; the importer reads them when you scan the model.
- **Preview before placing.** Wireframe of the model at its exact target position plus a bounding frame.
- **Watch it rise.** A speed slider, from 1 block per second to full speed, changes the pace while the model is
...
3. Assign a block, detail level and colour mode to each material. Reorder with the arrows.
4. Set scale, axes, rotation and offset. **Voxelize** and check the wireframe in the world.
5. **Place**. A progress bar shows blocks done and time left, the **Speed** slider sets the pace. **Undo** if needed, **Redo** to put it back.
