...
public final class ImportJob {
    ...
    /** Runs placement (or undo) until the budget is used up. Returns how many queued blocks were processed. */
    public int tick(int blockBudget, long deadlineNanos) {
        if (state == State.UNDOING) return tickUndo(blockBudget, deadlineNanos) ? 1 : 0;
        if (state != State.RECEIVING) return 0;
        int n = 0;
    ...
    public void updateBar(EntityPlayerMP player) {
        ...
        if (!undoing && speedBps > 0) sb.append("   ").append(speedBps).append(" bl/s");
    ...
    /** Blocks per second chosen with the GUI slider; 0 = as fast as the server budget allows. */
    public int speedBps;
    /** Fractional blocks this job may still place; lets slow speeds place one block every few ticks. */
    private double allowance;

    public void setSpeed(int index) {
        speedBps = ImportSpeed.blocksPerSecond(index);
        allowance = Math.min(allowance, 1.0);
        phaseStartMillis = 0; // the time-left estimate restarts at the new pace
    }

    /** Blocks this tick may place: the server budget, limited by the chosen speed. */
    public int speedBudget(int serverBudget) {
        if (speedBps <= 0) return serverBudget;
        double perTick = speedBps / 20.0;
        allowance = Math.min(allowance + perTick, Math.max(1.0, perTick * 2));
        return Math.min(serverBudget, (int) allowance);
    }

    public void consumeAllowance(int processed) {
        if (speedBps > 0) allowance = Math.max(0, allowance - processed);
    }
