...
public final class ImportSession {
    ...
    private void run(File file, ImportSettings settings) {
        try {
            ...
            if (setups.stream().allMatch(s -> s.skip)) {
                finish(Stage.ERROR, "All materials are skipped: untick Skip on at least one");
                return;
            }
    ...
    /** Speed slider step used for the next upload (see ImportSpeed). */
    public int speed = ru.arthaix.meshtiles.common.ImportSpeed.MAX_INDEX;
