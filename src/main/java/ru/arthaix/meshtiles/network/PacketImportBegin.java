...
public class PacketImportBegin extends CreativeCorePacket {
    ...
    /** Speed slider step (see ImportSpeed). */
    public int speed = ImportSpeed.MAX_INDEX;
    public ImportPalette palette;

    public PacketImportBegin() {}

    public PacketImportBegin(int sessionId, int grid, int totalBlocks, long totalBoxes, int speed, ImportPalette palette) {
        this.speed = speed;
    ...
    public void readBytes(ByteBuf buf) {
        ...
        speed = ImportSpeed.clamp(buf.readInt());
    ...
    public void executeServer(EntityPlayer player) {
        ...
        ImportJob job = ImportJobManager.INSTANCE.begin(mp, sessionId, grid, totalBlocks, totalBoxes, palette);
        if (job != null) job.setSpeed(speed);
    }
}
