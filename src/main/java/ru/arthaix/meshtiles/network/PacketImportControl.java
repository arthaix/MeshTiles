...
public class PacketImportControl extends CreativeCorePacket {
    ...
    /** An action with a small integer argument (the speed slider step). */
    public PacketImportControl(int action, int value) {
        this.action = action;
        this.x = value;
    }
    ...
    public void executeServer(EntityPlayer player) {
        ...
        else if (action == SPEED) ImportJobManager.INSTANCE.setSpeed(mp, (int) x);
